...
module.exports = {
    ...
    methods:{
        ...
        gettime: function(dateStr){
            var arr = []; 
            dateStr.replace(/\d+/g, function($i){ arr.push(parseInt($i, 10)); });
            return arr;
        },
        //计算几个月后的年与月
        addmonths: function(y, m, addmonth){
            var total = m+addmonth, next = total > 12, prev = total<1;
        ...
        isBetween: function(y, m, d, min, max){
            ...
            if(min){
                ...
                var by = bdate[0], bm = bdate[1], bd = bdate[2];
                if(y < by){
                    between=false;
                }else if(y === by){
                    if(m < bm){
            ...
            if(max){
                ...
                var ey = edate[0], em = edate[1], ed = edate[2];
                if(y > ey){
                    between=false;
                }else if(y === ey){
                    if(m > em){
        ...
        calcPane:function(y, m){
            var that=this, options, line=0, temp=[]; m = m - 1;
        ...
        select:function(k0,k1,k2,e){
            var that = this, date = this.days[k0][k1][k2];
            this.day=date.day;
